 ...
 * From http://home.hiwaay.net/~taylorc/toolbox/geography/geoutm.html
 */
var XutmXYToLatLon = function(x, y, zone, southhemi, latlon) {
    var cmeridian;
...
// Function to return the cardinal and intercardinal points based on supplied bearing
export function degToCard(deg) {
    if (deg < 0) {
        deg = 360.0 + parseInt(deg);
    }
...
// Converts milliseconds of time to nicely formatted days, hours and minutes for things like 'last seen'
export function dhm(t) {
    var d = Math.floor(t / MILLIS_PER_DAY);
    var h = Math.floor((t - d * MILLIS_PER_DAY) / MILLIS_PER_HOUR);
    var m = Math.round((t - d * MILLIS_PER_DAY - h * MILLIS_PER_HOUR) / 60000);
    ...
    if (h > 0) {
        output = h + ' hour ' + m + ' minute';
        if (m > 1) {
            output = h + ' hour ' + m + ' minutes';
        }
    }
    if (h > 1) {
        output = h + ' hours ' + m + ' minute';
        if (m > 1) {
            output = h + ' hours ' + m + ' minutes';
        }
    }
    if (d > 0) {
        if (h > 0) {
        ...
        if (h > 1) {
            output = d + ' day ' + h + ' hours';
        }
    }
    if (d > 1) {
        if (h > 0) {
            output = d + ' days ' + h + ' hour';
        }
        if (h > 1) {
            output = d + ' days ' + h + ' hours';
        }
    }
    return output + ' ago';
}

// use leaflet turf library to create a polygon that represents a circle around the point given
// resolution is number of points around the circle (eg 36 makes one point every 10 degrees around the cirle)
export function createCirclePolygon(pt, radius, units, resolution) {
    var ring = [];
...
export function utmXYToLatLon(x, y, zone, southhemi, latlon) {
    var cmeridian;
